...
- If any domain folder under `docs/ai_agent/` holds >8 active docs or a typical feature update touches 3+ domain files, migrate to the capability-based knowledge base (Option 3). Track counts with `find docs/ai_agent -mindepth 2 -maxdepth 2 -name '*.md' | awk -F/ '{print $(NF-1)}' | sort | uniq -c`.
