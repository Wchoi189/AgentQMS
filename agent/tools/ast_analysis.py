...
def agent_ast_analysis():
    ...
    try:
        ...
        try:
            from scripts.ast_analysis_cli import main
        ...
        try:
            from streamlit_app.utils.path_utils import get_project_root

            project_root = get_project_root()
            print(f"   Debug: Project root: {project_root}")
            print(
                f"   Debug: Scripts dir exists: {Path(project_root / 'scripts').exists()}"
            )
        except Exception:
            print("   Debug: Could not determine project root")
        return 1
