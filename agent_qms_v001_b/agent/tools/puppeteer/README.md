...
```json
{
  "mcpServers": {
    "puppeteer": {
      "command": "python3",
      "args": [
        "/workspaces/upstage-prompt-hack-a-thon-dev/agent/tools/puppeteer/agent_puppeteer_mcp.py"
      ],
      "env": {
...
1. **Verify the MCP server starts**:
   ```bash
   python3 agent/tools/puppeteer/agent_puppeteer_mcp.py
   ```
...
- Ensure Puppeteer wrapper is accessible:
  ```bash
  python3 -c "from scripts.agent_tools.utilities.puppeteer_wrapper import capture_page; print('OK')"
  ```
...
### Permission Issues
- Make the script executable: `chmod +x agent/tools/puppeteer/agent_puppeteer_mcp.py`
