...
This directory is the **Agent-Only Interface Layer** that provides convenience commands for AI agents. All actual tool implementations live in `scripts/agent_tools/`.

**Relationship**:
- `agent/` = Thin wrapper layer (Makefile, wrappers, config)
- `scripts/agent_tools/` = Implementation layer (actual Python scripts)
...
- **USE** `scripts/agent_tools/` directly for tool implementations

### **🤖 Agent Usage**

AI agents should:

1. **Navigate to this directory**: `cd agent/`
...
│   ├── discover.py      # Wraps scripts/agent_tools/core/discover.py
│   ├── feedback.py      # Wraps scripts/agent_tools/utilities/agent_feedback.py
│   └── quality.py       # Wraps scripts/agent_tools/compliance/documentation_quality_monitor.py
...
**Note**: All actual implementations are in `scripts/agent_tools/`. This directory provides convenience wrappers.
...
- Use tools in `scripts/agent_tools/` directly
- Do NOT use this agent directory
- See `scripts/agent_tools/README.md` for tool usage

**For AI Agents**:
- Use this directory for all automation tasks (`cd agent/` then `make help`)
