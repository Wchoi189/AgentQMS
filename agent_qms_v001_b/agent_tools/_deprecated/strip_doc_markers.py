...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Manage AI documentation markers in source files."
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "--apply", action="store_true", help="Remove markers and save snapshot."
    )
    group.add_argument(
        "--dry-run", action="store_true", help="Preview files containing markers."
    )
    group.add_argument(
        "--restore",
        action="store_true",
        help="Restore markers from the latest snapshot.",
    )
